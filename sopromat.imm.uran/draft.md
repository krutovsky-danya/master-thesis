...
Хоть в компьютере числа хранятся с конечной точностью, упомянутый фактор делает пространстов игры необъятным, поэтому можно считать его бесконечным.
...
В рамках игры количество карточек ограничено небольшим числом, поэтому пространстов действий конечно.
...
В игре можно выйграть, победить - набрать миллион условных денег или проиграть - опуститься до нуля.
...
Из-за отсутсвия эпизодичности некоторые алгоритмы RL могут быть не пременимы (Monte-Carlo) или плохо сходиться.
...
На этом этапе агент ограничен по количеству спринтов и должен максимизировать набор параметров, такие как благосостояние и довольность пользователей.
...
Гипотиза: если агент будет выбирать только из доступных действий, обучение ускорится и достигнет качственно лучшего результата.
...
Для достижения цели проекта нужно добится того, чтобы обученый агент попал в лидерборд.
...
Эта логика перенесена с использованиемм оригинального кода.

При валидации алгоритма - шанс выйграть в игре состовляет 68 процентов.
При этом минимальный выйгрышный спринт - 47.
...
Или же попробовать применить более сложние алгоритмы RL.

Можно равзить симулятор владельца продукта, немного приблизив его к реальности.
И с помощью текущих нараработок адаптировать ИИ побежадть в следующией версии игры.
